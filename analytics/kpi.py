...
def build():
    ...
    # Cost by DC
    cost_by_dc = (
        flows.groupby("dc_id")["flow_cost_usd"]
        .sum()
        .reset_index()
        .sort_values("flow_cost_usd", ascending=False)
    )
    cost_by_dc.to_csv(RESULTS_DIR / "cost_by_dc.csv", index=False)

    # Stores served
    served = (
        flows.groupby("dc_id")["store_id"]
        .nunique()
        .reset_index()
        .rename(columns={"store_id": "stores_served"})
    )
    ...
    cost_by_region = (
        flows_region.groupby("region")["flow_cost_usd"]
        .sum()
        .reset_index()
        .sort_values("flow_cost_usd", ascending=False)
    )
    ...
    flows_serv = flows.merge(
        transport[["dc_id", "store_id", "service_time_days"]],
        on=["dc_id", "store_id"],
        how="left",
    )
        ...
        [
            {
                ...
                "pct_units_on_slow_lanes": float(
                    100.0
                    * slow["units_assigned"].sum()
                    / max(flows["units_assigned"].sum(), 1.0)
                ),
            }
        ]
    )
...
def load_kpis():
    """Load KPI summary and related metrics, casting values to floats when possible."""
    ...
    # Add slow-lane KPIs
    try:
        slow = pd.read_csv(RESULTS_DIR / "slow_lanes_kpis.csv").iloc[0].to_dict()
        out["pct_units_on_slow_lanes"] = float(slow["pct_units_on_slow_lanes"])
        out["units_on_slow_lanes"] = float(slow["units_on_slow_lanes"])
    except Exception:
        pass

    # Add unmet demand summary
    try:
        unmet = pd.read_csv(RESULTS_DIR / "unmet_demand.csv")["unmet_units"].sum()
        out["unmet_units"] = float(unmet)
    except Exception:
        out["unmet_units"] = 0.0
